...
#
# Usage (repo root):
#   ./scripts/release-build.sh              # 기본: 폐쇄망 번들 dist/logmng-offline-${VERSION}.tar.gz (+ bin/)
...
#   VERSION — offline 번들 이름/폴더 (기본 1.0.0)
...
case "$MODE" in
  ...
  -h|--help|help)
    ...
    echo "  (default) offline — Full air-gap tarball under dist/ + bin/ (npm + mvn + tar)"
