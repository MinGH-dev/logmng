...
3.5. **Browser check (required when frontend was in scope; optional when backend/DB only)**  
   ...
   - **Steps**: Navigate to `http://localhost:3001`; capture page (snapshot or screenshot); confirm app shell (sidebar, main area, or login). Run §3 critical-path actions and, if the requirement doc has **§3.5 browser automation verification**, execute those procedures per TC. Use short waits (1–3 s) and capture again if the page is still loading.  
   ...
   - **Tool choice**: **Default (preferred)**: **AgentDeskAI** (user-browser-tools) — `takeScreenshot`, `runAccessibilityAudit`, `runPerformanceAudit`, `runSEOAudit` when available (Chrome extension + `npx @agentdeskai/browser-tools-server@latest`). For navigate/click/fill: prefer **puppeteer_*** (server-puppeteer, see `.cursor/mcp.json`) — CSS selectors, no ref dependency. If only **cursor-ide-browser**: **always** `browser_navigate` first, then `browser_lock`, then `browser_snapshot` (never lock before navigate); wait 2–3 s after navigate before snapshot; if snapshot returns only metadata (no refs), wait and retry once.
