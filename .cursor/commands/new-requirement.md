...
1. **Do not write** the requirement doc in this chat. **Invoke the Requirements subagent** via the Task tool (`subagent_type="Requirements"`). Pass the user request (or paste it below) and instruct Requirements to author the doc per `docs/workflow/REQUIREMENTS-AUTHORING-WORKFLOW.md` (parallel input from experts, orchestrate into §1·§2, **run change target verification** per `docs/workflow/REQUIREMENTS-CHANGE-TARGET-CHECKLIST.md` before finalizing §2, finalize §3). After the requirement doc exists, proceed to Step 2/3/4 as needed.
