...
| **Agent (Subagent)** | A **role** (Frontend, Backend, QA, …). Implemented as: (1) **agent definition** (Cursor metadata); (2) **prompt** (text pasted in Cursor Settings). | Definition: `.cursor/agents/*.mdc`<br>Prompt text: `docs/cursor-subagents/*.md` | User switches to Subagent, or main agent invokes via Task tool |
| **Main agent** | Default chat agent. Does not implement steps that have a dedicated Subagent; **delegates** (Task tool or instructs user to switch). | — | User talks in default chat |
| **Delegation** | Main agent handing work to a Subagent (by Task tool or by telling the user to switch and what to pass). Throughout docs, "mcp_task" = Cursor's Task tool. See `SUBAGENT-DELEGATION.md` §2.2 for mapping. | Rules: `agent-collaboration.mdc`<br>Docs: `SUBAGENT-DELEGATION.md`, `AGENT-COLLABORATION-ON-REQUIREMENT.md` | Main agent (or user manually) |
