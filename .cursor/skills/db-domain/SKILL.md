...
- **PB FEP wire + partitioning (req 20260414)**: `schema_pb_fep.sql` defines **wire-aligned** `pb_send`/`pb_recv` (varchar wire fields, `log_timestamp` as typed `TIMESTAMP NOT NULL` partition key, `wire_ts` for legacy quoted `"timestamp"`, `created_at`/`updated_at` for triggers). `migrate-pb-send-recv-partitioning-20260408.sql` converts ordinary tables to **daily** RANGE on `log_timestamp` plus DEFAULT attach, window **CURRENT_DATE − 30 … + 7** days. `migrate-pb-send-recv-monthly-to-daily-20260414.sql` is a **one-time** monthly→daily upgrade (idempotent if no `*_YYYYMM` children). Both run from `setup.sh` in order (`5-pb-fep-partition` then `5-pb-fep-partition-daily-upgrade`). **Operator-only (not in setup.sh)**: `create-pb-send-recv-daily-partitions-only.sql` pre-creates the daily window for already-partitioned parents and is **strict (no DEFAULT partition; out-of-range INSERT fails)**—also used for greenfield/ordinary conversion when run manually; embedded DDL matches wire columns. See `backend/DB_SETUP_GUIDE.md` § PB FEP partitioning.
