...
- **PB FEP partitioning (req 20260414)**: `migrate-pb-send-recv-partitioning-20260408.sql` converts ordinary `pb_send`/`pb_recv` to **daily** RANGE partitions (`pb_*_YYYYMMDD`) plus DEFAULT, window **CURRENT_DATE − 30 … + 7** days. `migrate-pb-send-recv-monthly-to-daily-20260414.sql` is a **one-time** monthly→daily upgrade (idempotent if no `*_YYYYMM` children). Both run from `setup.sh` after 20260408 in order (`5-pb-fep-partition` then `5-pb-fep-partition-daily-upgrade`). See `backend/DB_SETUP_GUIDE.md` § PB FEP partitioning.
