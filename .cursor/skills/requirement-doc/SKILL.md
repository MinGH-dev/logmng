...
Write requirement docs per the rules below. **Do not write §1 (user scenario, expected outcome) and §2 (codebase summary, problem analysis, solution) from Requirements' sole judgment.** Instead: obtain **parallel** input from experts and from codebase investigation; **orchestrate** (merge) that input into §1·§2; then finalize §3. **Read** `docs/workflow/REQUIREMENTS-AUTHORING-WORKFLOW.md` for the full procedure (§1.1, §1.2, §1.4). The §2 list is **planned change file list (expected change targets)**; do not use "Actual files changed" as the section name when authoring; the implementing agent (Step 4) **confirms or amends** it when implementation is done (REQUIREMENTS-AUTHORING-WORKFLOW.md §1.2). When authoring §2 and the change file list, use **requirement tone** (must, verify, align, confirm); avoid "No change", "already present", or "confirmed by" in the initial draft. For search/filter or layout alignment, ensure §2 **references** design docs for numeric and structural values; do not define new standards only in the requirement doc. When authoring §2.4 requirements (search/filter UI consistency), include in the Implementation note the full undefined-standard response rule (list undefined items, explain why needed, propose a draft, request feedback); see REQUIREMENTS-CHANGE-TARGET-CHECKLIST.md §2.4 and `docs/design/ux-frontend-standard-principles.md` §2. When aligning activity-log and statistics (or other user-context screens), include in §1 Expected outcome an **explicit** bullet that **user block fields** (department, user name, user ID) have the **same width/size** on all aligned screens; in §2/change list cover applying the same user-block width and ensuring layout does not squeeze the user block (for example, avoid sharing a single `1fr` cell); in §3 include at least one TC comparing user-block field size across screens. **Before finalizing §2**, run the **§2.4 verification table** in REQUIREMENTS-CHANGE-TARGET-CHECKLIST.md ("§2.4 verification (mandatory when pattern applies)") and confirm every row. See REQUIREMENTS-CHANGE-TARGET-CHECKLIST.md §2.4 "Field width by role (user block)" and `docs/workflow/ANALYSIS-user-field-size-activity-log-vs-statistics.md`.
...
- **Current year and date**: Read **`.cursor/CURRENT-DATE-CONVENTION.md`** and use the **current year** (and month/day rules) from that file for:
  ...
  - **In-document dates** (§5 test run date, authored date, completed date, verification date, etc.)
