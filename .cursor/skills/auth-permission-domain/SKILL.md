...
- The shared auth `selfContext` contract is `department`, `username`, `userId`. In API/UI, **userId** is **numeric** `app_user.id` (e.g. 20269999, 20260001). **`auth.login.mode`** (`application.yml` / **`application-{profile}.yml`**) selects exactly one path: **`local`** — `POST /api/auth/login` body **`userId`** (number) + **`password`** (vs `password_hash`); **`ad`** — body **`principal`** (string) + **`password`**; directory check is **JNDI simple bind** on **`auth.ad.ldap-url`** with UPN = principal if it contains `@`, else **`principal + "@" + auth.ad.domain`** (env **`AUTH_AD_DOMAIN`**). **`manager-dn` / `user-search-*`** are legacy optional YAML keys and **not** used for bind. After AD success, **`app_user`** is resolved via **`ExternalIdentityService.findAppUserIdForDirectoryPrincipal`** using the **request principal** (trimmed), not replacing it with the UPN, then **`app_user_external_identity`** / **`ext_employee`**. Misconfiguration (invalid mode, `ad` without required **`auth.ad.ldap-url`** / **`auth.ad.domain`**) is **fail-closed** (startup failure). `username` in selfContext is the **display name** (사용자명): `app_user.name` when present and not blank, otherwise `app_user.username`.
