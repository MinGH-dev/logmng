...
| 경로 | 설명 |
|------|------|
| `backend/logmng-backend-1.0.0.jar` | Spring Boot fat JAR. *`package-airgap-bin.sh` 실행 후 생성* (Git 무시) |
| `backend/run.sh` | 백엔드 기동 |
...
| `frontend/run.sh` | UI 기동(기본 포트 3001) |
...
```bash
java -jar logmng-backend-1.0.0.jar --spring.config.additional-location=file:./
```
