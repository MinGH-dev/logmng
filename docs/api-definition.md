...
- **Response (data)**: Same envelope as §5.1 — object with:
  - **`data`**: `object[]` — each row is a **wireframe-keyed** map (**key names stable**; values sourced from wire-format columns per contract):
    ...
    - **Keyword match hints (optional booleans, `keywords` active):** When **`keywords`** is non-empty, each row **may** include **`keyword_match_request_data`**, **`keyword_match_response_data`**, **`keyword_match_bmsg`**, **`keyword_match_data`** — each **`true`** iff at least one keyword token matched that logical column (or the **`data`** summary branch per `docs/contract.md` / spec) under the same rules as PB FEP keyword row filtering; values are **flags only**, not decrypted payload text. When **`keywords`** is absent or empty, servers **should omit** these keys **or** set all to **`false`**; clients treat **omission** like **`false`**. Normative: `specs/log-db-pb-fep-log-search.spec.yaml` (Keyword match hints), `docs/contract.md` (PB FEP wireframe search API table).
