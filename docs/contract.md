...
- **권한 그룹 관련 테이블** (요건 20250227, 20250303): `permission_group` (id, code, name, description, sort_order), `app_user_permission_group` (user_id = app_user.username, permission_group_id → permission_group.id), `permission_group_screen` (permission_group_id, screen_id, scope, read, write, approve — 모두 BOOLEAN NULL; scope='self'|'team'|'all', activity-log·statistics·search-history에만 적용, 생략/NULL 시 기본값 'team'; read/write/approve는 화면별 명시적 체크박스 저장, NULL=derived). 상세: `specs/permission-group-hierarchy.spec.yaml` §2.1, schema.sql.
