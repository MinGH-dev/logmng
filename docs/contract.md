...
**Requirement:** `docs/requirements/20260414-pb-fep-wire-schema-alignment.md`.
...
- **Time range filter behavior (no `log_timestamp` dependency):** `startDate`/`endDate` are interpreted on canonical `log_time` semantics. Backend may parse/normalize wire time values (`log_time`, `prc_time`, or agreed wire-time sources) at query time for comparison, but this normalization is transient execution logic only and not a schema/API field.
...
|-----------------|-------------------------------------|
| `log_time` | Canonical product-facing wire time field |
| `media_code` | **`media_gb`** |
| `tr_code` | **`tr_code`** |
...
**Requirements**: `docs/requirements/20260326-pb-fep-log-search-screen-wireframe.md` (§2.G, §2.D); wire schema / alias rules: `docs/requirements/20260414-pb-fep-wire-schema-alignment.md` (§2).
