...
| 프론트엔드 (검증 URL) | http://localhost:3001 | **Docker Compose** 정적 번들만 이 포트를 쓴다. 호스트 CRA·`dev-services.sh` 기본값은 **3002**(`FRONTEND_PORT` / `frontend/.env.development`의 `PORT`). |
...
- **UI 수동 검증(계약 기준)**: **`http://localhost:3001`** — Docker Compose 정적 프론트(dist 번들) 및 본 문서·슬래시 커맨드 검증과 동일한 기준 포트다.
- **선택: 로컬 CRA 개발 서버**: **`npm start`** 및 **`./scripts/dev-services.sh frontend`** 기본은 **포트 3002** (`package.json`의 `PORT=3002`, `frontend/.env.development`). **3001에 호스트 개발 서버를 띄우지 않는다** — Docker UI와 충돌한다. 계약·통합 검증은 **3001**(Compose) 또는 필요 시 `FRONTEND_PORT`로 다른 포트를 명시한다. CRA 기본 **3000**도 프로젝트 표준이 아니다.
...
### PB FEP — wire schema, partition key, and stable JSON aliases

**Requirements:** `docs/requirements/20260414-pb-fep-wire-schema-alignment.md`; **`log_time` lexical format (20-char microseconds):** `docs/requirements/20260415-pb-fep-log-time-20-char-microseconds.md`; **PB FEP keyword row filter + decrypt-for-match + list JSON:** `docs/requirements/20260415-pb-fep-keyword-decrypt-and-plaintext-search.md` §2; **browser-visible search JSON / DevTools / no client plaintext channel:** `docs/requirements/20260415-encrypted-field-search-no-client-plaintext.md` (PB FEP search inherits the same separation: match may use in-memory decrypt; list responses do not become a plaintext channel unless this Contract documents an exception).

**Physical tables (`pb_send` / `pb_recv`):** DDL follows the **legacy PB FEP wire-format** column layout (product reference export style, e.g. `pb_recv_*`). Representative wire columns include `log_time`, `log_ch_cd`, `log_io_cd`, `tr_code`, `brodid`, `media_gb`, `msg_code`, `pub_ip`, `prt_ip`, `bmsg`, `data`, and other wire fields as in the product list; binary/text payload columns follow the wire spec. **`log_timestamp` is physically removed from PB FEP tables and must not be reintroduced** in schema, migration, or API-contract assumptions.

**Logical/wire-facing schema policy (product-facing):** PB FEP product-facing logical schema is wire-first. **Canonical product-facing time field is `log_time`**.

**`log_time` wire value (normative for PB FEP / `pb_feplog`):**
- **Lexical string:** Exactly **20** characters: **decimal digits only**, no separators — pattern **`yyyyMMddHHmmssSSSSSS`** (8 + 6 + 6): calendar date, time to seconds, then **microseconds** as **six** digits, **zero-padded** (`SSSSSS`).
- **Examples:** `20260415143025123456` denotes 2026-04-15 14:30:25.123456. Start-of-calendar-day lower bounds align with **`…000000000000`** as the fractional part (12 trailing zeros in the full 20-digit string for “midnight” in this encoding).
- **Database:** Physical **`log_time`** on **`pb_send`** / **`pb_recv`** stores this wire form (**`VARCHAR(20)`** or product-equivalent width). **RANGE** partition bounds and row values must share **consistent width and padding** so **lexicographic** string order matches chronological order for normalized rows.
- **API — search (`startDate` / `endDate`):** For **`logType=pb_feplog`**, the backend applies the requested datetime range to **`log_time`** by **normalizing** bounds into this **same 20-character lexical space** and comparing **`log_time`** with **string (lexical) predicates** (e.g. `>=` / `<` on the padded wire strings). Client-facing **`startDate`/`endDate`** remain clock/datetime inputs; **no** API field reintroduces **`log_timestamp`**.
- **API — response echo:** Search and detail responses that expose PB FEP rows echo **`log_time`** as this **canonical 20-digit wire string** (stable JSON keys on **`POST .../search`** and wireframe keys on **`POST .../pb-fep-log-search`**), consistent with the physical column when normalized.

**Backend/Frontend implementation policy (normative):**
- **Request/filter/sort canonical key:** `log_time`.
- **Response canonical key:** `log_time`.
- **Time range filter behavior (no `log_timestamp` dependency):** `startDate`/`endDate` are interpreted on canonical `log_time` semantics **in the 20-character lexical format above** for PB FEP. Backend may parse/normalize wire time values (`log_time`, `prc_time`, or agreed wire-time sources) at query time for comparison, but this normalization is transient execution logic only and not a schema/API field.
- **Sort behavior (no alias):** default sort is `log_time desc`; `sortSpecs` fields must be allowlisted contract keys and must not include `log_timestamp`.
- **No removed-field usage:** frontend/backend/docs/specs must not request, emit, sort, filter, or map `log_timestamp`.

**Compatibility policy (normative):**
1. This requirement finalizes PB FEP `log_timestamp` physical/API-contract removal.
2. PB FEP contracts do not provide request/response/sort/filter compatibility aliases for `log_timestamp`.
3. If an external client still depends on removed keys, compatibility must be handled outside PB FEP contract (adapter/versioning) via a separate requirement.

**`loginId` filter:** Request **`loginId`** applies **equality on wire `brodid`** (the stored broker/login key). This replaces filtering on the old simplified column name `user_id` while preserving the same **semantic** for operators.

**Legacy `POST /api/logs/db-refactored/search` (`logType=pb_feplog`) — backward-compatible JSON keys:** Row objects keep **unchanged key names** for existing UIs and tests. The server selects or maps **wire physical columns** to these stable keys (SQL `AS` aliases and/or Java mapping), including at minimum:

| Stable JSON key | Physical / wire source (normative) |
|-----------------|-------------------------------------|
| `log_time` | Canonical product-facing wire time field — **20-digit** lexical **`yyyyMMddHHmmssSSSSSS`** (see **`log_time` wire value** above) |
| `media_code` | **`media_gb`** |
| `tr_code` | **`tr_code`** |
| `user_id` | **`brodid`** (same logical “user/broker key” as before; key name unchanged for clients) |

Additional legacy keys (`ip_address`, `request_data`, `response_data`, `error_message`, `status_code`, `session_id`, `device_type`, …) continue to be populated from the agreed wire/binary columns (`pub_ip`, `data`, `bmsg`, `msg_code`, …) per backend mapping; **`sortSpecs` / `sortField`** allowlists accept **legacy sort alias names** (e.g. `brodid`, `prc_time`, `bmsg`, `pub_ip`) and resolve them to the correct physical columns (**no SQL injection**).

**Wireframe route (below):** Response **`login_id`** is sourced from **`brodid`** (same value as legacy **`user_id`** on `/search`). Other wireframe fields are sourced from wire columns as in the wireframe mapping row in the table below.
...
#### PB FEP (`pb_feplog`) — keyword search, `decryptData`, list JSON, prefetch cap (normative)

**Requirements:** `docs/requirements/20260415-pb-fep-keyword-decrypt-and-plaintext-search.md` §2; **search vs browser exposure:** `docs/requirements/20260415-encrypted-field-search-no-client-plaintext.md` (inherit: decrypt-for-match allowed only to evaluate predicates; **Network JSON** for search/list must not carry decrypted PII for protected payload fields; optional non-plaintext match hints only if documented here and in `docs/api-definition.md`).

**Endpoints (same semantics):** **`POST /api/logs/db-refactored/search`** with **`logType=pb_feplog`** and **`POST /api/logs/db-refactored/pb-fep-log-search`**. When **`keywords`** is non-empty, tokens participate in **row inclusion** (not highlight-only): the returned **`data`** rows and **`pagination`** (**`totalCount`**, **`totalPages`**, current page) must reflect the **keyword-filtered** result set combined with structured SQL filters as **`(SQL filters) AND (keyword clause)`**.

**Keyword clause (default product rules — document any change in Contract + §3 tests):**

- **OR across list elements:** a row matches if **any** keyword token matches on **any** eligible surface.
- **Case rule (default):** **case-insensitive** substring match, aligned with **`java_fw_imglog`** unified text behavior unless the product explicitly chooses otherwise.

**Surfaces evaluated per row (token may hit any one):**

- **Plaintext / wire-as-stored strings** after the same JDBC→string rules the server uses for row values, including at least legacy **`request_data`**, **`response_data`**, **`error_message`** (wire **`bmsg`**) on **`POST .../search`**, and the wireframe row’s logical **`data`**, **`bmsg`**, and optional **`request_data` / `response_data`** on **`POST .../pb-fep-log-search`** (key names per wireframe mapping).
- **Decrypt-for-match (PB FEP payload):** when a match may exist only inside encrypted-at-rest payload text, the server may decrypt **in memory solely to evaluate the keyword predicate**, using the product’s PB FEP log-payload crypto variant (same **search vs display** separation as **`java_fw_imglog`**). **INFO/WARN** logs must not emit decrypted sensitive payload content from these match paths (see encrypted-field-search requirement).

**`decryptData` request field (`LogDbSearchRequest`):**

- **`decryptData` does not gate keyword matching** for **`pb_feplog`**: when **`keywords`** is non-empty, keyword filtering (including decrypt-for-match) **runs regardless of `decryptData`** (same principle as **`java_fw_imglog`**: match-time decrypt is not toggled off by `decryptData: false`).
- Clients **must not** be required to send **`decryptData: true`** only to obtain keyword hits.
- **`decryptData`** is **not** contract approval to populate **`decrypted_request_data`**, **`decrypted_response_data`**, or any other **`decrypted_*`** plaintext payload keys on **`POST .../search`** or **`POST .../pb-fep-log-search`** responses. **Default:** those keys **must not** appear on these search responses. Any exception (Security-approved, operator-visible plaintext on search wire) must be **explicitly** added to this Contract and **`docs/api-definition.md`** — absent that, implementers treat them as **forbidden** on both routes.

**Bounded prefetch (implementation option):** When **`keywords`** is non-empty, the backend **may** apply a **hard cap** on how many SQL-ordered candidate rows are scanned before in-memory keyword filtering (naming/size is implementer-defined, analogous to **`IMGLOG_FILTER_PREFETCH_CAP`** for **`java_fw_imglog`** text filters). If used:

- **`pagination`** values must be computed from the **same** keyword-filtered pipeline the contract defines for that implementation (no “SQL page then drop rows” totals that disagree with the visible row set).
- If the cap implies **incomplete visibility** (matches might exist outside the scanned window), the product **must** disclose operator-visible behavior (e.g. a **documented** response field or message) — **no silent wrong totals** relative to the documented guarantee.
- Exact field names and copy for any such disclosure are **DOC-CODE-SYNC** with `docs/api-definition.md` §5.1 / §5.1.1 when introduced.

### PB FEP wireframe search API (English contract summary)

**Requirements**: `docs/requirements/20260326-pb-fep-log-search-screen-wireframe.md` (§2.G, §2.D); wire schema / alias rules: `docs/requirements/20260414-pb-fep-wire-schema-alignment.md` (§2); **keywords + list JSON + `decryptData`:** `docs/requirements/20260415-pb-fep-keyword-decrypt-and-plaintext-search.md` §2; **no client plaintext from search:** `docs/requirements/20260415-encrypted-field-search-no-client-plaintext.md`; **keyword match flags (booleans only, wireframe rows):** `docs/requirements/20260416-pb-fep-keyword-match-flags-data-full-line-highlight.md` §2.

| Item | Definition |
|------|------------|
| **Path** | `POST /api/logs/db-refactored/pb-fep-log-search` (exact; wireframe screen **`pb-fep-log-search`** only). |
| **Purpose** | Return PB FEP union rows (`pb_send` / `pb_recv`, log type **`pb_feplog`**) with **wireframe-facing JSON keys** for the **`pb-fep-log-search`** UI. **`keywords`** (when non-empty) apply the **same row-filtering semantics** as **`POST .../search`** with **`logType=pb_feplog`** (see **PB FEP (`pb_feplog`) — keyword search…** above). |
...
| **Row object (wireframe keys)** | Each element of **`data`** uses **stable keys** for UI and **`expandedRowKeys`** (unchanged names). **`id`** (numeric row id), **`log_type`** or branch discriminator (**`send`** / **`recv`** or service-defined token with `id` for UNION uniqueness). **`log_time`** (canonical product-facing time field — **20-digit** lexical **`yyyyMMddHHmmssSSSSSS`** per **`log_time` wire value** above), **`tr_code`** from wire **`tr_code`**. **`login_id`** from wire **`brodid`** (same semantics as legacy `/search` **`user_id`**). **`msg_code`** from wire **`msg_code`** (display per formatter). **`bmsg`** from wire **`bmsg`** (or equivalent payload column per decrypt rules). **`log_ch_cd`** from wire **`log_ch_cd`** (or equivalent channel field). **`send_recv`**: **`SEND`** / **`RECV`** by union branch. **`src_ip`** from **`pub_ip`**. **`dest_ip`**: placeholder (empty or **`—`**) until a dedicated wire column is agreed. **`app_id`**: from agreed wire/session/term-related field(s), or empty. **`data`**: cell / stream summary from wire **`data`** and related payload columns per product and decrypt rules. Optionally **`request_data`** / **`response_data`** for expanded **STREAM DATA** when still used; decrypt targets follow wire column layout per §2. **Keyword match hints (optional booleans, no plaintext):** when **`keywords`** is non-empty, each row **may** include **`keyword_match_request_data`**, **`keyword_match_response_data`**, **`keyword_match_bmsg`**, **`keyword_match_data`** — **`true`** iff the corresponding wire column (or the **`data`** summary branch per requirement) contributed to keyword matching under the same rules as PB FEP keyword row filtering (wire substring or decrypt-for-match); **never** decrypted payload strings. When **`keywords`** is absent or empty, servers **should omit** these keys **or** send all **`false`**; clients treat **omission** like **`false`**. Normative detail: `specs/log-db-pb-fep-log-search.spec.yaml` (Keyword match hints). |
