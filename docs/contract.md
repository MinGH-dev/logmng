...
**Physical tables (`pb_send` / `pb_recv`):** DDL follows the **legacy PB FEP wire-format** column layout (product reference export style, e.g. `pb_recv_*`). Representative wire columns include `log_time`, `log_ch_cd`, `log_io_cd`, `tr_code`, `brodid`, `media_gb`, `msg_code`, `pub_ip`, `prt_ip`, `bmsg`, `data`, and other wire fields as in the product list; binary/text payload columns follow the wire spec. **Partitioning:** **RANGE** daily partitions use a single typed **`log_timestamp`** column (**`TIMESTAMP NOT NULL`**, or agreed **`TIMESTAMPTZ`** if timezone rules are explicit). Raw wire **`varchar`** time fields alone are not the partition key; **`log_timestamp`** is populated at insert/migration from `prc_time`, parsed `log_time`, and/or legacy quoted-identifier time columns per documented rules. Operational scripts (`create-pb-send-recv-daily-partitions-only.sql`, migration chain, `backend/DB_SETUP_GUIDE.md`) stay aligned with this column.
...
|-----------------|-------------------------------------|
| `log_timestamp` | Canonical typed **`log_timestamp`** (query range + partition key) |
| `media_code` | **`media_gb`** |
| `tr_code` | **`tr_code`** |
