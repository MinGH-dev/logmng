...
| **4** | **Backend / Frontend / DB** | Implementation in `backend/`, `frontend/`, or DB only — **including writing or modifying unit/integration test code** (e.g. JUnit, Jest). | Switch to **Backend**, **Frontend**, or **DB** subagent with requirement doc, §3, contract/spec, and any review output. They implement (including §3 test code), build, restart; **confirm or update** the requirement doc §2 **변경 파일 목록** (change file list) with the **actual** files changed; then hand off to QA. When they need detail the requirement doc does not specify and that belongs to an expert (UX, Contract, DBA, Security, Consistency), they **invoke that expert subagent** via the Task tool with a focused question (or ask the main agent to invoke); they do not assume. See `docs/workflow/REQUIREMENTS-AUTHORING-WORKFLOW.md` §1.2 (change file list), `docs/workflow/DEVELOPMENT-QUERY-EXPERTS.md` (query experts). |
