...
|------|----------|------------------|-------------------------------|
| **1** | **Requirements** | New requirement or error fix needs a formal requirement doc (§1, §2, §3). | Switch to **Requirements** subagent; provide the user request or error message. **During authoring**, Requirements **solicits feedback** from relevant expert subagents (Security, Contract, UX, DBA, Architecture, Consistency) per `docs/workflow/AGENT-COLLABORATION-ON-REQUIREMENT.md` §1.1; incorporates feedback into §1·§2, then finalizes §3. Output: requirement doc. **After doc is complete**, proceed to Step 2/3/4; each **responsible subagent** then performs its step in sequence. |
...
| **4** | **Backend / Frontend / DB** | Implementation in `backend/`, `frontend/`, or DB only. | Switch to **Backend**, **Frontend**, or **DB** subagent with requirement doc, §3, contract/spec, and any review output. They implement, build, restart, and hand off to QA. **When they need detail** the requirement doc does not specify and that belongs to an expert (UX, Contract, DBA, Security, Consistency), they **invoke that expert subagent** via mcp_task with a focused question (or ask the main agent to invoke); they do not assume. See `AGENT-COLLABORATION-ON-REQUIREMENT.md` §1.2. |
