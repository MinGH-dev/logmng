...
|------|----------|------------------|-------------------------------|
| **1** | **Requirements** | New requirement or error fix needs a formal requirement doc (§1, §2, §3). | Switch to **Requirements** subagent; provide the user request or error message. **During authoring**, Requirements uses a **hybrid approach** per `AGENT-COLLABORATION-ON-REQUIREMENT.md` §1.1: (1) read `.cursor/skills/` for domain baseline, (2) investigate codebase via SemanticSearch/Grep/Read, (3) invoke **only applicable experts** (`readonly=true`, `model="fast"`) — Security, Contract, DBA, Architecture, Consistency, UX — when the requirement touches their domain, (4) invoke RequirementsPastSearch when user has not explicitly changed prior scope; then **orchestrates** into §1·§2 (structured by scope for handoff) and finalizes §3 (with Scope tags). Output: requirement doc. **After doc is complete**, proceed to Step 2/3/4; each **responsible subagent** then performs its step in sequence. |
