...
3. **Codebase investigation (tools)**  
  ...
  - For shared table/grid footer issues, explicitly check whether one-page datasets are suppressing the **shared footer region** or footer metadata (`총 n건`, rows-per-page). Treat that as a shared footer-contract issue first, not as a multi-page-only pagination issue.
