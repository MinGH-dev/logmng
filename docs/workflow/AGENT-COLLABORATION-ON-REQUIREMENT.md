...
| **4** | **Backend / Frontend / DB** | Requirement doc, §3, contract/spec, reviews | Code and config; **implement unit/integration tests** that cover §3 test cases for all modified/added code (tests must run in `mvn test` / `npm test` or documented integration); **build and restart** (required). **Confirm or update** requirement doc §2 **변경 파일 목록** with actual files changed. When detail is missing, **query the owning expert subagent** (see §1.3). | Step 4.5 or 5 |
