...
|------|-----------|--------|--------|------------|
| **1** | **Requirements** | User request, error message, or feature need | Requirement doc with §1, §2, §3. **During authoring**: obtain **parallel** input from experts and Backend/Frontend/DB/QA (scenario, codebase, problem, solution); **orchestrate** into §1·§2; §2 "변경 파일 목록" is **tentative** (see REQUIREMENTS-AUTHORING-WORKFLOW.md §1.1, §1.2). | Step 2 if security-relevant; else Step 3 or 4 |
